...
function updateCartFooter() {
  ...
  const cartFooter = document.querySelector(".cart-price");

  if (cartFooter) {
    if (totalPrice > 0) {
      cartFooter.innerHTML = `<p class='total-price'>Total Price: $${totalPrice.toFixed(2)}</p>`;
    } else {
      cartFooter.innerHTML = "<p class='total-price'>Your cart is empty.</p>";
    }
  }
}
